...
from src.models.residual_moe import ClusterwisePredResidualMoE  # noqa: E402
...
def evaluate(
    backbone: nn.Module,
    residual: ClusterwisePredResidualMoE,
    loader: DataLoader,
    ...
    for x, y, _ in loader:
        ...
        mask = y_base.new_ones((y_base.shape[0], residual.K, residual.P))
        out = residual(x, y_base, cluster_id_c, mask)
        y_hat = out["y_final"]
        branch = y_hat - y_base
        base_err = y_base - y
        pred_err = y_hat - y
        ...
        branch_sq += float(branch.pow(2).sum().item())
        base_pred_sq += float(y_base.pow(2).sum().item())
        raw_residual_sq += float(out["residuals"].pow(2).sum().item())
        numel += int(y.numel())
    ...
    return {
        ...
        "branch_base_rms_ratio": (branch_sq / max(base_pred_sq, 1.0)) ** 0.5,
        "raw_residual_rms": (raw_residual_sq / max(numel, 1)) ** 0.5,
    }


def make_residual(meta: dict[str, Any], args: argparse.Namespace) -> ClusterwisePredResidualMoE:
...
def run_cell(cell: str, spec: dict[str, str], args: argparse.Namespace) -> dict[str, Any]:
    ...
    residual = make_residual(meta, args).to(device)
    batch_size = int(args.batch_size or cfg.get("train", {}).get("batch_size", 64))
    train_loader = make_loader(cfg, data_tc, "train", batch_size)
    val_loader = make_loader(cfg, data_tc, "val", batch_size)
    test_loader = make_loader(cfg, data_tc, "test", batch_size)
    optimizer_name = str(args.optimizer).lower()
    if optimizer_name == "adam":
        opt = torch.optim.Adam(residual.parameters(), lr=float(args.lr), weight_decay=float(args.weight_decay))
    ...
    for epoch in range(1, int(args.epochs) + 1):
        ...
        for x, y, _ in train_loader:
            ...
            with torch.no_grad():
                y_base = backbone(x, cluster_id_c)
            mask = y_base.new_ones((y_base.shape[0], residual.K, residual.P))
            y_hat = residual(x, y_base, cluster_id_c, mask)["y_final"]
        ...
        alpha = residual.alpha_values().detach().cpu().view(-1).tolist()
        if val_metrics["residual_mse"] < best_val_mse:
            best_val_mse = float(val_metrics["residual_mse"])
            best_state = copy.deepcopy(residual.state_dict())
        history.append({"epoch": epoch, "train": train_metrics, "val": val_metrics, "alpha": alpha})
        print(
            f"{cell} epoch={epoch} "
            f"alpha_mean={sum(alpha) / max(len(alpha), 1):.4f} "
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Control run using ClusterwisePredResidualMoE itself with P=1 and mask=1.")
    parser.add_argument("--cells", default="ETTh1_H96,ETTm2_H96")
    parser.add_argument("--out-json", default="outputs/anchorless_moe_diagnostic/residual_moe_module_control.json")
    ...
    parser.add_argument("--init-alpha", type=float, default=4.0)
    parser.add_argument("--alpha-scale", type=float, default=1.0)
    parser.add_argument("--feature-mode", default="legacy")
    ...
    result: dict[str, Any] = {
        "meta": {
            "purpose": "Train ClusterwisePredResidualMoE directly with one always-on penalty to isolate module capacity from train.py routing/loss selection.",
