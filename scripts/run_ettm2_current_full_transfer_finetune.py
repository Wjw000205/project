...
FIELDS = [
    ...
    "finetune_test_mae",
    "finetune_best_epoch",
    "finetune_val_gain_pct_vs_zero_shot",
    "finetune_vs_target_self_mse",
    "finetune_vs_target_self_mae",
    "finetune_gain_pct_vs_zero_shot",
    "selected_transfer_variant",
    "selected_transfer_val_mse",
    "selected_transfer_mse",
    "selected_transfer_mae",
    "selected_transfer_gain_pct_vs_zero_shot",
...
def build_zero_config(
    *,
    ...
    out_dir = out_root / "zero_shot" / f"{SOURCE}_to_{target}" / f"pred_{horizon}" / eval_split
    resample_enable = target in {"ETTh1", "ETTh2"} and resample_method.lower() not in {"none", "off", "false"}
    cfg = {
        "exp": {"name": f"{SOURCE}_to_{target}_H{horizon}_zero_shot_{eval_split}", "out_dir": str(out_dir), "seed": 2026, "device": device},
        "source": {
        ...
        "eval": {"batch_size": int(batch_size), "split": eval_split},
    }
    path = out_root / "configs" / "zero_shot" / f"{SOURCE}_to_{target}_H{horizon}_{eval_split}.yaml"
    write_yaml(path, cfg)
    return path
...
def build_finetune_config(
    *,
    ...
    if early_stop_patience is None:
        ...
        cfg["early_stop"]["patience"] = int(early_stop_patience)
...
def run_one(args: argparse.Namespace, best: dict[tuple[str, int], dict[str, Any]], target: str, horizon: int, lr: float) -> dict[str, Any]:
    ...
    zero_val_cfg = build_zero_config(
        source_cfg_path=source_cfg,
        source_summary_path=source_summary_path,
        checkpoint_path=ckpt,
        memory_path=memory,
    ...
    zero_val_out = Path(read_yaml(zero_val_cfg)["exp"]["out_dir"])
    zero_summary_path = zero_out / "transfer_summary.json"
    zero_val_summary_path = zero_val_out / "transfer_summary.json"
    ...
    if args.rerun or not zero_val_summary_path.exists():
        print(f"[zero-shot-val] {SOURCE}->{target} H{horizon}", flush=True)
        run_cmd([py, "-u", "-m", "src.transfer", "--config", str(zero_val_cfg)], log_path=zero_val_out / "zero_shot_val.log")
    zero = load_json(zero_summary_path)
    zero_val = load_json(zero_val_summary_path)
    ...
    row = {
        ...
        "zero_shot_val_mse": zero_val.get("avg_mse", ""),
        "zero_shot_val_mae": zero_val.get("avg_mae", ""),
    ...
    try:
        ...
        pass
    try:
        zs_val = float(row["zero_shot_val_mse"])
        ft_val = float(row["finetune_val_mse"])
        row["finetune_val_gain_pct_vs_zero_shot"] = (zs_val - ft_val) / zs_val * 100.0 if zs_val else ""
        if ft_val <= zs_val:
            row["selected_transfer_variant"] = "finetune"
            row["selected_transfer_val_mse"] = ft_val
            row["selected_transfer_mse"] = ft_mse
            row["selected_transfer_mae"] = ft_mae
            row["selected_transfer_gain_pct_vs_zero_shot"] = row.get("finetune_gain_pct_vs_zero_shot", "")
        else:
            row["selected_transfer_variant"] = "zero_shot"
            row["selected_transfer_val_mse"] = zs_val
            row["selected_transfer_mse"] = row["zero_shot_mse"]
            row["selected_transfer_mae"] = row["zero_shot_mae"]
            row["selected_transfer_gain_pct_vs_zero_shot"] = 0.0
    except Exception:
        pass
    return row
...
def main() -> None:
    ...
    ap.add_argument("--out-root", type=Path, default=ROOT / "outputs" / "ettm2_current_full_horizon_transfer_finetune")
