...
def read_run_metrics(run_dir: Path, label: str) -> Dict[str, Any]:
    ...
    dyn_cfg = load_yaml(str(run_dir / "config.yaml")).get("moe", {}).get("dynamic_lambda", {}) if (run_dir / "config.yaml").exists() else {}
    row = {
        "experiment": label,
        "run_dir": str(run_dir),
        "penalties": ",".join(summary.get("penalty_names", [])),
        "penalty_count": int(len(summary.get("penalty_names", []))),
        "dynamic_mode": str(dyn_cfg.get("mode", "")),
        "dynamic_max_factor": float(dyn_cfg.get("max_factor", 0.0)) if dyn_cfg else 0.0,
        "dynamic_mix": float(dyn_cfg.get("mix", 0.0)) if dyn_cfg else 0.0,
...
def main() -> None:
    ...
    ap.add_argument("--out-root", type=str, default="outputs/ETTm1/penalty_lambda_refine_20260420")
    ...
    existing_runs = {
        "add_amp_0p05_existing": resolve_path("outputs/ETTm1/penalty_lambda_ablation_20260420/add_amp"),
        "dynamic_liquid_3p0_1p0_existing": resolve_path("outputs/ETTm1/penalty_lambda_ablation_20260420/dynamic_liquid"),
    }
    for label, run_dir in existing_runs.items():
        if (run_dir / "run_summary.json").exists():
            results.append(read_run_metrics(run_dir, label=label))
    ...
    for exp in experiments:
        ...
        cfg.setdefault("exp", {})
        cfg["exp"]["name"] = f"penalty_lambda_refine_{name}"
