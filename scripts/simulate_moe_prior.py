...
def read_run_metrics(run_dir: Path, label: str, cfg_path: Path = None) -> Dict[str, Any]:
    ...
    if cfg_path is not None and cfg_path.exists():
        ...
        moe_cfg = cfg.get("moe", {})
        gate_prior = moe_cfg.get("gate_prior", {})
        row.update(
            {
                ...
                "gate_prior_enable": bool(gate_prior.get("enable", False)),
                "gate_prior_strength": float(gate_prior.get("logit_strength", 0.0)),
                "gate_prior_smoothing": float(gate_prior.get("smoothing", 0.0)),
                "gate_prior_temperature": float(gate_prior.get("temperature", 1.0)),
            }
        )
    row.update(stats)
    return row
...
def main() -> None:
    ...
    ap.add_argument("--base-config", type=str, default="outputs/ETTm1/moe_ablation_20260420/configs/top1_route_skip_cost_0p15.yaml")
    ap.add_argument("--baseline-run-dir", type=str, default="outputs/ETTm1/moe_ablation_20260420/top1_route_skip_cost_0p15")
    ap.add_argument("--out-root", type=str, default="outputs/ETTm1/moe_prior_ablation_20260420")
    ...
    experiments: List[Dict[str, Any]] = [
            ...
            "patch": {
                "moe": {
                    "gate_prior": {
                        ...
                        "use_as_balance_target": True,
                    },
                },
            },
        },
        {
            "name": "prior_s1_balance_no_entropy",
            "patch": {
                "moe": {
        ...
        {
            "name": "prior_s1p5_balance_no_entropy",
            "patch": {
                "moe": {
                    "gate_entropy_weight": 0.0,
                    "gate_prior": {
    ...
    results.append(read_run_metrics(baseline_run_dir, label="baseline_top1_route_skip_cost_0p15", cfg_path=base_config_path))
    ...
    for exp in experiments:
        ...
        cfg.setdefault("exp", {})
        cfg["exp"]["name"] = f"moe_prior_{name}"
        ...
        if summary_path.exists():
            results.append(read_run_metrics(out_dir, label=name, cfg_path=cfg_path))
        else:
