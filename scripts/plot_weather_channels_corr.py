...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        description="Plot Weather raw channel curves, global Pearson correlation matrix, and global clustering summary."
    )
    ap.add_argument("--config", type=str, default="configs/weather.yaml", help="YAML config path.")
    ...
    ap.add_argument(
        "--output",
        type=str,
        default="outputs/weather/weather_channels_corr_cluster.png",
...
def clean_label(name: str) -> str:
    text = str(name)
    text = text.replace("\ufffdmol/m\ufffds", "umol/m2/s")
    text = text.replace("W/m\ufffd", "W/m2")
    text = text.replace("\ufffd", "")
    return text
...
def wrap_assignments(channel_names: Sequence[str], cluster_ids: Sequence[int], per_line: int = 5) -> str:
    tokens = [f"{name}->C{int(cid)}" for name, cid in zip(channel_names, cluster_ids)]
    rows = []
    for i in range(0, len(tokens), per_line):
        rows.append(" | ".join(tokens[i:i + per_line]))
    return "\n".join(rows)
...
def main() -> None:
    ...
    display_names = [clean_label(name) for name in channel_names]
    data_norm_tc, _, _ = normalize_like_train(data_tc, cfg)

    start = int(args.start)
    stop = min(int(data_tc.shape[0]), start + int(args.points))
    if start < 0 or start >= int(data_tc.shape[0]):
        raise ValueError(f"start={start} is out of range for dataset length {data_tc.shape[0]}")
    if stop - start < 2:
    ...
    cluster_lines = cluster_summary_lines(clusters, display_names)
    assignments = wrap_assignments(display_names, cluster_id_c.detach().cpu().tolist(), per_line=5)

    fig = plt.figure(figsize=(18, 11))
    gs = fig.add_gridspec(
        nrows=2,
        ncols=2,
        height_ratios=[3.9, 1.7],
        width_ratios=[1.35, 1.0],
    )
    fig.subplots_adjust(left=0.05, right=0.98, top=0.94, bottom=0.07, hspace=0.35, wspace=0.28)
    ...
    extra_colors = [plt.get_cmap("tab10")(i) for i in range(10)]
    colors = [cmap(i % 20) if i < 20 else extra_colors[(i - 20) % len(extra_colors)] for i in range(len(display_names))]
    for idx, name in enumerate(display_names):
        ax_left.plot(x, y_plot[:, idx], linewidth=1.0, alpha=0.88, color=colors[idx], label=name)
    ...
    ax_left.set_title(f"Weather raw channels ({stop - start} plotted points, global clustering)")
    ax_left.grid(alpha=0.22, linestyle="--", linewidth=0.6)
    ax_left.legend(ncol=4, fontsize=8, frameon=True, loc="upper left")

    im = ax_right.imshow(corr_np, cmap="coolwarm", vmin=-1.0, vmax=1.0, aspect="equal")
    ax_right.set_xticks(np.arange(len(display_names)))
    ax_right.set_yticks(np.arange(len(display_names)))
    ax_right.set_xticklabels(display_names, rotation=45, ha="right", fontsize=8)
    ax_right.set_yticklabels(display_names, fontsize=8)
    ...
    for i in range(corr_np.shape[0]):
        for j in range(corr_np.shape[1]):
            ...
            ax_right.text(j, i, f"{corr_np[i, j]:.2f}", ha="center", va="center", fontsize=6, color=color)
    ...
    title += ")"
    body = "\n".join(cluster_lines + ["", assignments])
    ax_bottom.text(
        0.01,
        0.95,
        title,
    ...
    for line in cluster_lines:
        ...
        print(f"  {safe}")
    safe_assign = assignments.encode("gbk", errors="replace").decode("gbk")
    print(f"Assignments:\n{safe_assign}")
